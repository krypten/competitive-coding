...
void update(int node, int L, int R, int i, int new_value) {
	if(L==R && L==i) {
		T[node].maxv = T[node].sum = T[node].rval = T[node].lval = a[i] = new_value;
            //printf("%d\n", new_value);
		return;
	}
	int m = (L + R)/2;
	if(i<=m) update(2*node, L, m, i, new_value);
	else if(i>m) update(2*node+1, m+1, R, i, new_value);
