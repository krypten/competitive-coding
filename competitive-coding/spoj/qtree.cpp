/*
 *  Author		: Krypten
 *  Algorithm	: Heavy Light Decomposition (Source : namespace anudeep)
 */
...
vector <int> adj[SIZE], costs[SIZE], indexx[SIZE];
int baseArray[SIZE], ptr;
int chainNo, chainInd[SIZE], chainHead[SIZE], posInBase[SIZE];
int depth[SIZE], pa[LOG_SIZE][SIZE], otherEnd[SIZE], subsize[SIZE];
int st[SIZE*6], qt[SIZE*6];
...
int LCA(int u, int v) {
	...
	for(int i=0; i<LOG_SIZE; i++) if( (diff>>i)&1 ) u = pa[i][u];
	if(u == v) return u;
	for(int i=LOG_SIZE-1; i>=0; i--) if(pa[i][u] != pa[i][v]) {
		u = pa[i][u];
		v = pa[i][v];
	}
	return pa[0][u];
...
int main() {
	...
	while(t--) {
		...
		for(int i=0; i<n; i++) {
			...
			indexx[i].clear();
			chainHead[i] = -1;
			for(int j=0; j<LOG_SIZE; j++) pa[j][i] = -1;
		}
		for(int i=1; i<n; i++) {
			int u, v, c;
			scanf("%d %d %d", &u, &v, &c);
